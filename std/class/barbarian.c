...
mapping class_featmap(string myspec) {
    return ([ 1 : ({ "light armor proficiency", "medium armor proficiency", "shield proficiency", "simple weapon proficiency", "martial weapon proficiency", "rage", "powerattack", "unarmored defense" }), 2 : ({ "dodge" }), 5 : ({ "danger sense" }), 7 : ({ "damage resistance" }), 10: ({"damage reduction"}), 11 : ({ "greater rage"}), 14 : ({ "mobility" }), 17:({"tireless rage"}), 20 : ({ "mighty rage"}) ]);
}
