...
mapping class_featmap(string myspec) {
    return ([ 1 : ({ "light armor proficiency", "simple weapon proficiency", "martial weapon proficiency", "dodge" }), 4 : ({ "combat reflexes" }), 5 : ({ "danger sense" }), 7 : ({ "mobility" }), 10 : ({ "tools of the trade" }), 11 : ({ "evasion" }), 13 : ({ "use magic device" }), 14 : ({ "scramble" }), 17 : ({ "spring attack" }) ]);
}
