...
With the tower still under construction, it appears you cannot yet enter the gates. \n");
    ...
    set_items( ([
      "gates" : "%^RESET%^A %^CYAN%^strange magical force %^RESET%^seems to %^BOLD%^%^CYAN%^dance %^RESET%^between the %^BOLD%^%^BLACK%^bars %^RESET%^of the %^BOLD%^%^BLACK%^wrought iron gates%^RESET%^, suggesting that they are not as weak as they appear.  You get the feeling that these %^BOLD%^%^BLACK%^gates could withstand the might of any army.",
